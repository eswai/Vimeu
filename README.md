...
| 状態 | キー | 動作 |
|---|---|---|
| 入力中 | ローマ字 | 読みを入力。ライブ変換オン時はMozc 1位を表示し、候補判定をバックグラウンドで開始 |
| 入力中 | `Space` | ライブ変換オン時は判定後の候補一覧を表示。オフ時はMozc 1位をインライン表示 |
| 入力中 | `Enter` | 読み（またはライブ変換の結果）をそのまま確定 |
| 入力中 | `Delete` | 一文字削除 |
| 入力中 | `Esc` | 入力を全取消 |
| 入力中 | `,` / `.` | `、` / `。` |
| 変換中（ライブ変換オフ・候補一覧なし） | `Space` | 判定完了後に候補一覧を表示 |
| 候補選択中 | `↓` / `Space` | 次の候補 |
| 候補選択中 | `↑` | 前の候補 |
...
| **共起** | 正の共起（同時に選びたい）／負の共起（同時に出したくない）をドロップダウンで登録。登録済みの一覧と削除 |
| **接続** | 選択中の候補の各境界の接続コスト。単語と同じ 5 つの操作。効く範囲は品詞の組 |
...
- **共起** = 正の共起を満たしたペアぶんの割引（負の共起は候補を除外し、コスト項を持たない）
...
種別を「負の共起」にして `皮` と `剥く` を登録すると、その2語を同時に含む候補はコストに
関係なく除外される。負の共起は語順を問わない。正負が同じ2語で衝突したときは、後から登録した
種別が前の登録を置き換える。
...
`したい` が `死体` になるのは単語コストのせいではない。ひらがなの `したい`（動詞,自立,連用形）は
単語コストなら 3219 で、`死体` の 5160 より 1941 も安い。負けているのは
`BOS → 動詞,自立,連用形` が 3132、`BOS → 名詞,一般` が 1066 という**文の始まり方**の差であって、
語の重みではない。ここで `死体` を弱めると `死体` が出るべき文まで壊れる。

**接続**ペインは、その 1 つの遷移を直接動かす場所である。操作は単語ペインと同じ
（強める / 弱める / 直接編集 / 使わない（復活）/ リセット）で、刻みも同じ ∓500。
上の例は「強める」1 回で `したい` が 1 位になる。

ただし**効く範囲が違う**。単語の編集はその語が出る文にしか効かないが、接続の編集は
同じ品詞の組を通るすべての文に効く。この窓はそのうち 1 文しか見せられないので、影響を測るには
テストセットに掛ける:
...
`--user` は IME とまったく同じ有効辞書で走るので、2 つの数字の差が編集の影響そのものになる。
上の 1 件では 3 テストセット 25,775 文のうち動いたのは top-5 の 1 文だけだった
（[DESIGN.md](DESIGN.md) §4.2）。1 回の編集が動かすのは行列の 1 マスだけである。
...
接続の編集は品詞 ID ではなく品詞名（`id.def` の feature 文字列）で保存してあるので、
Mozc のデータが更新されて ID が振り直されても、同じ品詞の組に付いていく。
